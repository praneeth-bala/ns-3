 ...
 * \file
 * \ingroup simulator
 * ns3::SimbricksSimulatorImpl implementation.
 */
...
NS_LOG_COMPONENT_DEFINE ("SimbricksSimulatorImpl");

NS_OBJECT_ENSURE_REGISTERED (SimbricksSimulatorImpl);

TypeId
SimbricksSimulatorImpl::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SimbricksSimulatorImpl")
    .SetParent<SimulatorImpl> ()
    .SetGroupName ("Core")
    .AddConstructor<SimbricksSimulatorImpl> ()
  ;
  return tid;
}
...
void
SimbricksSimulatorImpl::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
SimbricksSimulatorImpl::Destroy ()
{
  NS_LOG_FUNCTION (this);
...
void
SimbricksSimulatorImpl::SetScheduler (ObjectFactory schedulerFactory)
{
...
uint32_t
SimbricksSimulatorImpl::GetSystemId (void) const
{
  return MpiInterface::GetSystemId ();
}

void
SimbricksSimulatorImpl::ProcessOneEvent (void)
{
  Scheduler::Event next = m_events->RemoveNext ();
...
bool
SimbricksSimulatorImpl::IsFinished (void) const
{
  return m_events->IsEmpty () || m_stop;
}

void
SimbricksSimulatorImpl::ProcessEventsWithContext (void)
{
...
void
SimbricksSimulatorImpl::Stop (void)
{
  NS_LOG_FUNCTION (this);
  m_stop = true;
}
...
void
SimbricksSimulatorImpl::Stop (Time const &delay)
{
...
void
SimbricksSimulatorImpl::ScheduleWithContext (uint32_t context, Time const &delay, EventImpl *event)
{
...
Time
SimbricksSimulatorImpl::Now (void) const
{
...
Time
SimbricksSimulatorImpl::GetDelayLeft (const EventId &id) const
{
  if (IsExpired (id))
    {
      return TimeStep (0);
...
void
SimbricksSimulatorImpl::Remove (const EventId &id)
{
  if (id.GetUid () == 2)
    {
      // destroy events.
...
void
SimbricksSimulatorImpl::Cancel (const EventId &id)
{
  if (!IsExpired (id))
    {
...
bool
SimbricksSimulatorImpl::IsExpired (const EventId &id) const
{
  if (id.GetUid () == 2)
    {
...
Time
SimbricksSimulatorImpl::GetMaximumSimulationTime (void) const
{
  return TimeStep (0x7fffffffffffffffLL);
}

uint32_t
SimbricksSimulatorImpl::GetContext (void) const
{
  return m_currentContext;
}

uint64_t
SimbricksSimulatorImpl::GetEventCount (void) const
{
  return m_eventCount;
}
