 ...
 */
#ifndef NET_CACHE_DEVICE_H
#define NET_CACHE_DEVICE_H
 ...
 * \file
 * \ingroup bridge
 * ns3::NetCacheDevice declaration.
 */
 ...
 *
 * The NetCacheDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a NetCacheDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class NetCacheDevice : public NetDevice
{
public:
  /**
   ...
   *
   * This method adds a new bridge port to a NetCacheDevice, so that
   ...
   * bridging node you must enable IP on the NetCacheDevice itself,
...
private:
   ...
   */
  NetCacheDevice (const NetCacheDevice &);
   ...
   * \returns
   */
  NetCacheDevice &operator = (const NetCacheDevice &);
