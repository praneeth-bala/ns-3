 ...
 */
#ifndef PEGASUS_DEVICE_H
#define PEGASUS_DEVICE_H
 ...
 *
 * The PegasusDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a PegasusDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class PegasusDevice : public NetDevice
{
public:
  /**
   ...
   *
   * This method adds a new bridge port to a PegasusDevice, so that
   ...
   * bridging node you must enable IP on the PegasusDevice itself,
...
protected:
  ...
  Ptr<const Packet> P4 (Ptr<const Packet> packet);
  ...
  void ipchecksum (uint16_t* base, int size, uint16_t* dest);
...
private:
   ...
   */
  PegasusDevice (const PegasusDevice &);
   ...
   * \returns
   */
  PegasusDevice &operator = (const PegasusDevice &);
  ...
  Mac48Address m_src48, m_dst48;
  uint32_t m_next_version;
  bool m_ver_matched;
