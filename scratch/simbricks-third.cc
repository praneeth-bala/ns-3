...
//dons2023: subnet scope under parallel and distributed simulation
uint32_t sub_network_left = 0;
uint32_t sub_network_right = 0;
uint32_t systemId = 0;
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		if(flow_input.src >= sub_network_left && flow_input.src <= sub_network_right){
			printf("LP %d: add flow %d to %d\n",systemId,flow_input.src,flow_input.dst);
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			else if (key.compare("TRACE_OUTPUT_FILE") == 0)
			{
				...
				if (argc > 5)
				{
					trace_output_file = trace_output_file + std::string(argv[5]);
				}
	...
	//dons2023: parallel and distributed simulation setup
	bool nullmsg = false;
	...
	// }
	GlobalValue::Bind("SimulatorImplementationType",
						StringValue("ns3::SimbricksSimulatorImpl"));
	...
	//uint32_t host_num_per_LP = (int)(host_num/systemCount);
	//uint32_t sw_num_per_LP = (int)(switch_num/systemCount);
	uint32_t host_num_per_LP = ((int)(host_num/systemCount) == 0 ? 1: host_num/systemCount);
	uint32_t sw_num_per_LP = ((int)(switch_num/systemCount) == 0 ? 1: switch_num/systemCount);
	...
	//dons2023: topology partitioning
	Ptr<UniformRandomVariable> LP_uv = CreateObject<UniformRandomVariable>();
	for (uint32_t i = 0; i < node_num; i++){
		int belongtoLP = 0;
		if (node_type[i] == 0) //host
		{
			belongtoLP = (int)(i/host_num_per_LP);
			if(belongtoLP >= systemCount)
				belongtoLP = systemCount - 1;
			//belongtoLP = i%systemCount; //32 LPs
		} else { 				//switch: randomly assigned switch to LPs
			//Split orizontally
			//if(i>=192)//192 8192+32*32  //Core
			//	belongtoLP = 3%systemCount;  //i%10+22;
			//else if((i-128)%8>=4)  		//Agg
			//	belongtoLP = 2%systemCount;  //i%11+11;
			//else							//ToR 
			//	belongtoLP = 1%systemCount;	//(i-1)%11+0;

			belongtoLP = i%systemCount; //32 LPs fattree4
			
			//Random splitting
			//belongtoLP = (int)(LP_uv->GetValue() * 1000)%systemCount;
			
			//Split vertically
			//belongtoLP = (int)((i-host_num)/sw_num_per_LP);
			//belongtoLP %= systemCount;
		}
		printf("LP %d topology partitioning: node %d to LP %d.\n",systemId,i,belongtoLP);
		if (node_type[i] == 0)
			n.Add(CreateObject<Node>(belongtoLP));
		else{
			Ptr<SwitchNode> sw = CreateObject<SwitchNode>(belongtoLP);
			n.Add(sw);
	...
	for (uint32_t i = 0; i < link_num; i++)
	{
		...
		char ipstring[24];
		sprintf(ipstring, "%d.%d.%d.0", i / 64516 + 1, (i / 254)%254 + 1, i % 64516 + 1);
	...
	flow_input.idx = 0;
	sub_network_left = systemId*host_num_per_LP;
	sub_network_right = (systemId+1)*host_num_per_LP-1;
	if (flow_num > 0){
		ReadFlowInput();
		Simulator::Schedule(Seconds(flow_input.start_time)-Simulator::Now(), ScheduleFlowInputs);
	}
	...
	Simulator::Run();
	Simulator::Destroy();
	MpiInterface::Disable(); //dons2023: Distributed simulation setup
	NS_LOG_INFO("Done.");
	fclose(trace_output);
