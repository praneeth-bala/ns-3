...
Client::StopApplication (void)
{
    ...
    std::flush(std::cerr);
    std::ofstream fs;
    std::string fname = "/workspaces/simbricks/experiments/out/netcache.client"+std::to_string(this->m_client_id);
...
main (int argc, char *argv[])
{
    ...
    Ipv4Address IpA ("10.0.0.10");
    Ipv4Address IpB ("10.0.0.11");
    ...
    servers.push_back(InetSocketAddress (Ipv4Address ("10.0.0.10"), 12345));
    servers.push_back(InetSocketAddress (Ipv4Address ("10.0.0.11"), 12346));
    ...
    setupClient(nodeC, clients[0], 0, 2, 3, 4, servers);
    setupClient(nodeD, clients[1], 1, 2, 3, 4, servers);
    setupClient(nodeE, clients[2], 2, 2, 3, 4, servers);
